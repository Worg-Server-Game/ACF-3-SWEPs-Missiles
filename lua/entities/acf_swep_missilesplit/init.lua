...
function MakeACF_SWEPATGMSplit(Gun, BulletData, EnableGuidance, HeatSeeking, HeatTarget)
	local Entity = ents.Create("acf_swep_missilesplit")
...
function ENT:Think()
	...
	local stupidViewConeAng = 90
	local stupidViewCone = math.cos(math.rad(stupidViewConeAng))
	...
	if self.UseGuidance then
		...
		if self.HeatSeeking then
			local AirBurstTargets = ACF.GetEntitiesInCone(Position - self:GetForward() * 1500, self:GetForward(), 90)

			for Entity in pairs(AirBurstTargets) do
				local EntPos  = Entity:GetPos()
				local EntDist = EntPos:Distance(Position)

				if EntDist > 4000 then continue end

				if self:GetForward():Dot((EntPos - Position) / EntDist) < math.rad(45) then
					self:Detonate(false, true)
				end
			end

			if self.HeatTarget and IsValid(self.HeatTarget) then
				if self.HeatTarget:GetPos():Distance(self:GetPos()) < 3000 then
					self:Detonate(nil, true)
					return
				end

				if (self.HeatTarget:GetPos() - Position):GetNormalized():Dot(self:GetForward()) > stupidViewCone then
					self.HeatTarget = nil 
				end
			end

			if not self.HeatTarget then
				local Targets = ACF.GetEntitiesInCone(Position, self:GetForward(), stupidViewConeAng)
				...
				for Entity in pairs(Targets) do
					...
					if Direction:Dot(self:GetForward()) > stupidViewCone then
						local LOSTraceData = {
							start = Position,
...
function ENT:Detonate(Destroyed, AirBurst)
	...
	if not AirBurst then
		...
		local initialFlight = BulletData.Flight
		BulletData.Agility = BulletData.Agility * 5
		BulletData.ViewConeAng = 175
		BulletData.ViewCone = math.cos(math.rad(BulletData.ViewConeAng))
		for i = 1, 15 do
			BulletData.Flight = initialFlight + Vector(math.Rand(-800, 800), math.Rand(-800, 800), math.Rand(-800, 800))
			local missile = MakeACF_SWEPATGM( self, self.BulletData, true, true, self.HeatTarget )
			table.insert(self.BulletData.Filter, missile)
		end
		self:SetPos(prevPos)
	end
